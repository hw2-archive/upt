...
function readJson(file, options) {
    ...
    function readCustom(meta) {
        ...
        var customFile=path.join(options.config.cwd,options.config.directory, depName,"bower.custom.json");
        try {
    ...
    .spread(function (json, jsonFile) {
        ...
        if (err.code === 'ENOENT' && options.assume) {
            ...
            var meta=readCustom(bowerJson.parse(options.assume, options));
