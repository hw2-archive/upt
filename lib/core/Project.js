...
function Project(config, logger) {
    ...
    this._manager = new Manager(this._config, this._logger);
     ...
     this._options = shared.set("options",{});
}
...
Project.prototype.install = function (decEndpoints, options, config) {
    ...
    this._options = shared.set("options",options || {});
    ...
    .spread(function (json, tree) {
        // It shows an error when issuing `upt install`
        // and no upt.json is present in current directory
        if(!that._jsonFile && decEndpoints.length === 0 ) {
            throw createError('No upt.json present', 'ENOENT');
        }
...
Project.prototype.saveJson = function (forceCreate) {
    ...
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn('no-json', 'No upt.json file to save to, use upt init to create one');
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'upt.json');
