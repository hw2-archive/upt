...
var endpointParser = require('hw2core-endpoint-parser');
...
Manager.prototype.resolve = function () {
    ...
    .fin(function () {
        ...
        for (var dynDep in this._pendingDyn) {
            dynDep=this._pendingDyn[dynDep];
            if (dynDep["realName"])
                this._writeDep(dynDep);
        }
...
Manager.prototype.install = function (json) {
    ...
    .then(function () {
        ...
        mout.object.forOwn(that._dissected, function (decEndpoint, name) {
            ...
            var oldMetaFile = path.join(dst, 'bower.json');

            var filters=[];
            if (fs.existsSync(oldMetaFile)) {
                var contents=fs.readFileSync(oldMetaFile,"utf-8");
                if (contents)
                    filters= JSON.parse(contents.toString()).keep || [];
            }
            ...
            for (var df in depFilters) {
                ...
                if (df.indexOf(name)===0) {
                    var str=df.replace(new RegExp("^"+name+"/","g"),'');
                    if (str != df) {
                        filters.push(str);
                    }
                }
            ...
            promise = Q.nfcall(rmrf.purgeDeploy,dst, filters)
...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
    ...
    if (decEndpoint._parent && decEndpoint._dynName) {
        this._dynamicDep[decEndpoint._dynName].realName=decEndpoint.name;

        if (this._changeDep(decEndpoint._parent.pkgMeta,"dependencies",decEndpoint._dynName,decEndpoint.name) 
        || this._changeDep(decEndpoint._parent.pkgMeta,"devDependencies",decEndpoint._dynName,decEndpoint.name)) {
            var json = JSON.stringify(decEndpoint._parent.pkgMeta, null, '  ');
            fs.writeFileSync(path.join(decEndpoint._parent.canonicalDir,".bower.json"), json);
        }
    }
    ...
    var options=cli.readOptions({ 'production': { type: Boolean, shorthand: 'p' } });
...
Manager.prototype._parseDependencies = function (decEndpoint, pkgMeta, jsonKey) {
    ...
    mout.object.forOwn(pkgMeta[jsonKey], function (value, key) {
        ...
        if (key[0]==="%") {
            ...
            for (var dynDep in this._dynamicDep) {
                var dynDep=this._dynamicDep[dynDep];
                if (dynDep["source"]===value) {
                    // add additional info for write
                    dynDep.pkgMeta=pkgMeta;
                    dynDep.jsonkey=jsonKey;
                    dynDep.dynName=key;
                    dynDep.canonicalDir=decEndpoint.canonicalDir;
            ...
            // create dynamicDep 
            this._dynamicDep[key] = {"source": value};
...
Manager.prototype._changeDep = function(pkgMeta,jsonkey,oldName,newName) {
    if (!pkgMeta[jsonkey] || !pkgMeta[jsonkey][oldName])
        return false;

    var tmp=pkgMeta[jsonkey][oldName];
    delete pkgMeta[jsonkey][oldName];
    pkgMeta[jsonkey][newName]=tmp;

    return true;
}

Manager.prototype._writeDep = function(dynDep) {
    if (this._changeDep(dynDep.pkgMeta,dynDep.jsonkey,dynDep.dynName,dynDep.realName)) {
        var json = JSON.stringify(dynDep.pkgMeta, null, '  ');
        fs.writeFileSync(path.join(dynDep.canonicalDir,".bower.json"), json);
    }
}
