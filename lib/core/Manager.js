...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
    ...
    if (decEndpoint.name[0]=="%") {
        var specialName=decEndpoint.name;
        decEndpoint.name = name = pkgMeta.name;  

        this._resolved[specialName]=this._resolved[specialName] || [];
        this._resolved[specialName].push(decEndpoint);
    ...
    // Parse dependencies
    this._parseDependencies(decEndpoint, pkgMeta, 'dependencies', skipRecursion);
...
Manager.prototype._parseDependencies = function (decEndpoint, pkgMeta, jsonKey, skipRecursion) {
    var pending = [];
