...
Manager.prototype.resolve = function () {
    ...
    .fin(function () {
        ...
        for (var dynDep in this._pendingDyn) {
            ...
            if (dynDep["realName"])
                this._changeDep(dynDep,dynDep.jsonKey);
        }
        ...
        mout.object.forOwn(that._dissected, function (decEndpoint, name) {
            ...
            if (fs.existsSync(metaFile)) {
                ...
                if (contents) {
                    ...
                    if (json.keep)
                        filters= json.keep.slice(0);
                }
            }

            // add custom json that must remain after updates
            filters.push("bower.custom.json");
...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
    ...
    if (decEndpoint._parent && decEndpoint._dynName) {
        ...
        newMeta.dynName=decEndpoint._dynName;
        newMeta.realName=decEndpoint.name;
        ...
        if (key[0]==="%") {
            ...
            for (var dynDep in this._dynamicDep) {
                ...
                if (dynDep["source"]===value) {
                    ...
                    if (dynDep.realName)
                        this._changeDep(dynDep,jsonKey);
                    else
 ...
 * @returns {Boolean}
 */
Manager.prototype._changeDep = function(dynDep,jsonKey) {
    var meta=dynDep.pkgMeta[jsonKey];

    if (!meta || !meta[dynDep.dynName])
        return false;

    var tmp=meta[dynDep.dynName];
    delete meta[dynDep.dynName];
    meta[dynDep.realName]=tmp;
