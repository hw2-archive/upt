...
var hook = function (action, ordered, config, logger, packages, installed, json) {
    ...
    orderedPackages.forEach(function(pkg){
        var hookScript = packages[pkg].canonicalDir +"/_hw2/installer_hook.js"

        if (fs.existsSync(hookScript)) {
            // maybe we should create a child fork for it instead requiring ?
            hooks.push(Q.fcall(function(action,config,pkg,packages,packages,installed) {
                var deferred=Q.defer();
                require(hookScript)(action,config,pkg,packages[pkg].canonicalDir,packages[pkg],installed[pkg],deferred.resolve);
                return deferred.promise;
            },action,config,pkg,packages,packages,installed));
        };
    });
