...
var hook = function (action, ordered, config, logger, packages, installed, json) {
    ...
    // RUN hook scripts ( now only implemented for postinstall and preuninstall )
    if (action=="postinstall" || action=="preuninstall") {
        orderedPackages.forEach(function(pkg){
            var pkgPath = path.join(config.cwd,config.directory,pkg);
            var hookScript = pkgPath +"/_hw2/installer_hook.js"

            if (fs.existsSync(hookScript)) {
                // maybe we should create a child fork for it instead requiring ?
                require(hookScript)(action,config.cwd,pkgPath);
            };
        });
    }
