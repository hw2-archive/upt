...
Resolver.prototype._readJson = function (dir) {
    ...
    return readJson(dir, {
        ...
        var meta=res[0], deprecated=res[1], assumed=res[2];

        var customFile=path.join(that._config.cwd,that._config.directory,meta.name,"bower.custom.json");
        try {
            var customMeta = require(customFile);
            if (customMeta) {
                //meta=deepmerge(meta,customMeta);
                // replace values instead merge
